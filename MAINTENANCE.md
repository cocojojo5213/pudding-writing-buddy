...
`CODE_AUDIT.md` 里记录的最近一次完整验证是 100 个测试通过。只要工作树是脏的，这条记录就只能当历史记录，必须重新跑验证。
...
模型 Base URL 也不能包含 URL 用户名或密码；API key 只填写在模型配置的 key 字段里。
...
- `/api/settle` 现在不会把数组/对象/布尔值章节 id 字符串化成已保存章节 id，从而借用旧正文。
- 当前完整测试为 100 个通过。
