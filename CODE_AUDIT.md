...
Result: 37 tests passing, including core library tests and real HTTP API subprocess tests. Runtime checks on `http://127.0.0.1:5179` also verified health, static `POST`/`HEAD` behavior, Host-header-safe static parsing, malformed static path handling, non-object JSON rejection, empty-settle rejection, unique snapshots, sanitized export filenames, clean project restoration, and desktop/mobile screenshots.
