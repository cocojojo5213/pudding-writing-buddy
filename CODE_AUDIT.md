...
- `server.js` lines 204-256: write routes now serialize `versionToken` checks and disk writes through one in-process queue. Concurrent same-version saves cannot both pass the check; stale project, settle, and reset writes return `409` instead of overwriting newer work. Blind writes without any version marker are also rejected when a project already exists. `/api/settle` now uses the current disk project as its write base after the version check, so stale client project payloads cannot overwrite saved title, notes, or chapter metadata during settlement.
...
Result: 39 tests passing, including core library tests and real HTTP API subprocess tests. Runtime checks on `http://127.0.0.1:5179` also verified health, static `POST`/`HEAD` behavior, Host-header-safe static parsing, malformed static path handling, non-object JSON rejection, empty-settle rejection, unique snapshots, sanitized export filenames, clean project restoration, and desktop/mobile screenshots.
